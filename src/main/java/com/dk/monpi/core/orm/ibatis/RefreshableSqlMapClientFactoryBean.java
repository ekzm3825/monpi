...
public class RefreshableSqlMapClientFactoryBean extends SqlMapClientFactoryBean implements SqlMapClientRefreshable, DisposableBean {
	...
	private static final Logger log = LoggerFactory.getLogger(RefreshableSqlMapClientFactoryBean.class);
...
//					}
		...
		List mappingLocationList = extractMappingLocations(configLocations, sqlMapClientProperties);
	...
	private List extractMappingLocations(Resource[] configLocations, Properties properties) {
		...
		for (int i = 0; i < configLocations.length; i++) {
			try {
				...
				mappingLocationList.addAll(configParser.parse(is, properties));
			}
			catch (IOException ex) {
				log.warn("Failed to parse config resource: "
